...
services.AddOpenIddict()
    ...
    .AddClient(options =>
    {
        ...
        {
            Issuer = new Uri("https://localhost:8781/", UriKind.Absolute)
        });
    });
...
static async Task CreateAccountAsync(IServiceProvider provider, string email, string password)
{
    ...
    var response = await client.PostAsJsonAsync("https://localhost:8781/Account/Register", new { email, password });
